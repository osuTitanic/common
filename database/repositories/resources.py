...
@session_wrapper
@ttl_cache(ttl=60)
def fetch_all(session: Session = SessionProvider) -> List[DBResourceMirror]:
...
@session_wrapper
@ttl_cache(ttl=60)
def fetch_all_by_server(server: int, session: Session = SessionProvider) -> List[DBResourceMirror]:
    return session.query(DBResourceMirror) \
...
@session_wrapper
@ttl_cache(ttl=60)
def fetch_by_type(type: int, server: int, session: Session = SessionProvider) -> List[DBResourceMirror]:
    return session.query(DBResourceMirror) \
...
@session_wrapper
@ttl_cache(ttl=60)
def fetch_by_type_all(type: int, session: Session = SessionProvider) -> List[DBResourceMirror]:
    return session.query(DBResourceMirror) \
        .filter(DBResourceMirror.type == type) \
        .order_by(DBResourceMirror.priority.asc()) \
